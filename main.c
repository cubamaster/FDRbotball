...
int up = 1450; // standard upper position for the lifting servo 
int open = 1630; // standard open position for the claw 
int close = 2007; // standard closed open position for the claw
...
void follow_line_1 (double coefficient, int position) {
    cmpc(rw);
    cmpc(lw);
    int i = 0;
    while(i <= position) {
...
void follow_line_2 (double coefficient) {
    while(1 == 1) {
        if (analog(2) > 3000) { // case if robot went over the black tape 
            motor(rw, -100);
            motor(lw, 100 * coefficient);
        } // close if function 
